...
// All interactive elements share a 28px height for visual rhythm.
const TopBar = ({ layout = "default", onLayout = () => {}, onInvite = () => {}, invitesLeft = 3 }) => {
  const InvitePill = window.InvitePill;
  const H = 28; // shared control height
  return (
  <div style={{ height:52, background:"#fff", borderBottom:"1px solid var(--rule)", display:"flex", alignItems:"center", padding:"0 14px", gap:10, fontSize:13, color:"var(--ink)" }}>
    <Mark size={32} colorN="var(--indigo)" colorS="var(--teal)"/>
    <div style={{ width:1, height:20, background:"var(--rule)", margin:"0 4px" }}/>
    {/* Project switcher */}
    <button className="row" style={{ height:H, padding:"0 10px", borderRadius:7, fontSize:12, lineHeight:1, gap:6, display:"inline-flex", alignItems:"center" }}>
      <span style={{ fontWeight:500, lineHeight:1, display:"inline-block" }}>{PROJECT.name}</span>
      <Icons.chevDown size={14}/>
    </button>
    {/* Layout-mode switcher */}
    <div style={{ height:H, display:"flex", alignItems:"center", padding:2, background:"var(--paper-2)", borderRadius:8, border:"1px solid var(--rule)" }}>
      {[
        ["default","Default"],
        ["reading","Reading"],
        ["drafting","Drafting"],
        ...
        <button key={id} onClick={() => onLayout(id)}
          style={{ height:H - 6, padding:"0 10px", fontSize:11.5, borderRadius:6, border:"none", background: layout === id ? "var(--ink)" : "transparent", color: layout === id ? "var(--paper)" : "var(--ink-2)", cursor:"pointer", fontFamily:"inherit", whiteSpace:"nowrap" }}>
          {label}
        </button>
      ))}
    </div>
    ...
    {/* Right cluster */}
    {InvitePill && <InvitePill count={invitesLeft} onClick={onInvite} h={H}/>}
    <button className="ns-btn ghost" style={{ height:H, padding:"0 10px", fontSize:12, gap:6 }}><Icons.share size={12}/> Share</button>
    <button className="ns-btn" style={{ height:H, padding:"0 12px", fontSize:12, gap:6 }}><Icons.sparkles size={12}/> New session</button>
    <div style={{ width:H, height:H, borderRadius:H/2, background:"var(--indigo)", color:"#fff", display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:600, marginLeft:2 }}>JP</div>
  </div>
  );
};
...
const SidePanel = ({ width = 280 }) => (
  <div className="pane" style={{ width, borderRadius:0, borderLeft:"none", borderTop:"none", borderBottom:"none", background:"var(--paper)", height:"100%" }}>
    ...
    <div style={{ display:"flex", gap:6, padding:"0 14px 8px", flexWrap:"wrap" }}>
      ...
      <span className="tag" style={{ fontSize:10.5, gap:4 }}><Icons.starFill size={10}/> Starred</span>
