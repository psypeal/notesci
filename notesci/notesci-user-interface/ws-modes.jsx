...
const { InviteModal: WSInviteModal } = window;
...
const ModeDefault = ({ graphMode = "citations", setGraphMode = () => {}, layout = "default", onLayout = () => {}, inviteOpen: inviteOpenProp }) => {
  const [inviteOpen, setInviteOpen] = React.useState(false);
  const open = inviteOpenProp !== undefined ? inviteOpenProp : inviteOpen;
  return (
    <Frame>
      <TopBar layout={layout} onLayout={onLayout} onInvite={() => setInviteOpen(true)} invitesLeft={3}/>
      ...
      {open && WSInviteModal && <WSInviteModal onClose={() => setInviteOpen(false)} contained={true}/>}
    </Frame>
  );
};
