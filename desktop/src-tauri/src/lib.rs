...
:root{color-scheme:light;}*{box-sizing:border-box;-webkit-app-region:no-drag!important;}html,body{margin:0;width:100%;height:100%;overflow:hidden;overscroll-behavior:none;}body{display:grid;place-items:center;background:radial-gradient(circle at 14% 18%,rgba(206,225,178,.9) 0 12%,transparent 34%),radial-gradient(circle at 88% 4%,rgba(190,210,231,.86) 0 14%,transparent 38%),radial-gradient(circle at 74% 86%,rgba(239,219,166,.62) 0 15%,transparent 38%),linear-gradient(135deg,#fbf5e3 0%,#edf5e8 48%,#e5eef4 100%);font-family:Georgia,'Times New Roman',serif;color:#1f2a24;}
body:before{content:'';position:fixed;inset:-18%;background:conic-gradient(from 145deg,transparent 0 18%,rgba(35,57,42,.07) 20% 28%,transparent 31% 58%,rgba(122,143,92,.09) 63% 70%,transparent 73%);animation:drift 9s linear infinite;}body:after{content:'';position:fixed;inset:0;background-image:linear-gradient(rgba(31,42,36,.035) 1px,transparent 1px),linear-gradient(90deg,rgba(31,42,36,.035) 1px,transparent 1px);background-size:44px 44px;mask-image:radial-gradient(circle at 50% 48%,black 0 58%,transparent 82%);}
.card{position:relative;width:min(680px,calc(100vw - 42px));padding:34px;border:1px solid rgba(54,68,55,.16);border-radius:32px;background:linear-gradient(180deg,rgba(255,255,255,.86),rgba(255,255,255,.72));box-shadow:0 32px 92px rgba(37,52,42,.18),inset 0 1px 0 rgba(255,255,255,.72);backdrop-filter:blur(20px);overflow:hidden;animation:card-in .45s cubic-bezier(.2,.8,.2,1) both;}
.card:before{content:'';position:absolute;inset:0;border-radius:inherit;background:linear-gradient(110deg,transparent 0 32%,rgba(255,255,255,.44) 46%,transparent 60%);transform:translateX(-120%);animation:sheen 2.8s ease-in-out infinite;pointer-events:none;}.top{position:relative;display:flex;align-items:center;gap:18px;margin-bottom:18px;}
.mark{width:54px;height:54px;border-radius:20px;background:linear-gradient(145deg,#1f3427,#466044);position:relative;box-shadow:inset 0 0 0 1px rgba(255,255,255,.2),0 16px 34px rgba(31,42,36,.2);}.mark:before{content:'';position:absolute;inset:8px;border-radius:999px;border:1px solid rgba(247,229,162,.45);}.mark:after{content:'';position:absolute;inset:13px;border:3px solid #f7e5a2;border-left-color:transparent;border-bottom-color:rgba(247,229,162,.28);border-radius:999px;animation:spin .82s linear infinite;}
.eyebrow{margin:0 0 5px;font:11px/1.2 ui-monospace,SFMono-Regular,Consolas,monospace;letter-spacing:.16em;text-transform:uppercase;color:#66736a;}h1{margin:0;font-size:32px;letter-spacing:-.03em;}p{position:relative;margin:9px 0 0;color:#536159;font:15px/1.65 system-ui,-apple-system,sans-serif;}
.rail{position:relative;height:9px;border-radius:999px;background:rgba(38,59,45,.12);overflow:hidden;margin-top:26px;box-shadow:inset 0 1px 2px rgba(31,42,36,.08);}.rail:before{content:'';position:absolute;inset:0 auto 0 0;width:46%;border-radius:inherit;background:linear-gradient(90deg,#263b2d,#9baf69,#e7d38b,#263b2d);animation:load 1.45s ease-in-out infinite;}.rail:after{content:'';position:absolute;inset:0;background:linear-gradient(90deg,transparent,rgba(255,255,255,.55),transparent);transform:translateX(-100%);animation:scan 1.9s ease-in-out infinite;}
.stage-list{position:relative;display:grid;grid-template-columns:repeat(3,1fr);gap:9px;margin:19px 0 0;padding:0;list-style:none;font:11px/1.35 ui-monospace,SFMono-Regular,Consolas,monospace;color:#6a766e;text-transform:uppercase;letter-spacing:.08em;}.stage-list li{display:flex;align-items:center;gap:7px;padding:9px 10px;border-radius:14px;background:rgba(38,59,45,.055);border:1px solid rgba(54,68,55,.08);transition:background .22s,border-color .22s,color .22s,transform .22s;}.stage-list li:before{content:'';width:7px;height:7px;border-radius:999px;background:#9aa69d;box-shadow:0 0 0 3px rgba(154,166,157,.12);}.stage-list li.is-active{color:#263b2d;background:rgba(122,143,92,.16);border-color:rgba(122,143,92,.28);transform:translateY(-1px);}.stage-list li.is-active:before{background:#7a8f5c;animation:pulse 1.1s ease-in-out infinite;}
.hint{position:relative;margin-top:15px;color:#6a766e;font:12.5px/1.5 system-ui,-apple-system,sans-serif;}@keyframes spin{to{transform:rotate(360deg);}}@keyframes load{0%{transform:translateX(-112%);}100%{transform:translateX(236%);}}@keyframes scan{0%,35%{transform:translateX(-100%);}100%{transform:translateX(100%);}}@keyframes drift{to{transform:rotate(360deg);}}@keyframes pulse{50%{box-shadow:0 0 0 6px rgba(122,143,92,.18);}}@keyframes sheen{45%,100%{transform:translateX(120%);}}@keyframes card-in{from{opacity:0;transform:translateY(10px) scale(.985);}to{opacity:1;transform:none;}}@media (prefers-reduced-motion:reduce){*,*:before,*:after{animation-duration:.001ms!important;animation-iteration-count:1!important;transition:none!important;}}
...
<div class="rail" aria-hidden="true"></div><ul class="stage-list" aria-hidden="true"><li data-stage="database" class="is-active">database</li><li data-stage="backend">backend</li><li data-stage="workspace">workspace</li></ul>
<div class="hint">Everything stays local. First launch may initialize storage once; later launches reuse it.</div></main>
<script>window.notesciSetStartupStatus=function(title,detail){var t=document.getElementById('startup-title');var d=document.getElementById('startup-detail');if(t&&title)t.textContent=title;if(d&&detail)d.textContent=detail;var text=((title||'')+' '+(detail||'')).toLowerCase();var active=text.indexOf('backend')>=0?'backend':text.indexOf('opening')>=0||text.indexOf('interface')>=0||text.indexOf('workspace')>=0?'workspace':'database';document.querySelectorAll('[data-stage]').forEach(function(el){el.classList.toggle('is-active',el.getAttribute('data-stage')===active);});};</script>
</body></html>"#;
    let script = format!(
